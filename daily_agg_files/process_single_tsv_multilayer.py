...
if not gpkg_file.exists():
    print(f"Error: GeoPackage file not found: {gpkg_file}")
    sys.exit(1)

print(f"Processing: {tsv_file.name}")
print(f"Using GeoPackage: {gpkg_file.name}")
...
try:
    stats = process_tsv_file(tsv_file, output_dir, by_polygon=by_polygon, gpkg_path=gpkg_file)
